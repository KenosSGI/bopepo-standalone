...
abstract class AbstractCampoLivre extends BlockOfFields implements CampoLivre {
	 ...
	 * Looger.
	 */
	private static Logger log = LogManager.getLogger(Objects.class);
		...
		try{
			...
			if (BancosSuportados.isSuportado(titulo.getContaBancaria().getBanco().getCodigoDeCompensacaoBACEN().getCodigoFormatado())) {
				...
				switch (banco) {
					...
					case C6_BANK: return AbstractCLC6Bank.create(titulo);
					case BANCO_MONEY_PLUS: return AbstractCLBancoMoneyPlus.create(titulo);
