 ...
 *
 */
public class TestCLBancoMoneyPlus extends AbstractCampoLivreBaseTest<CLBancoMoneyPlus> {
	
	@Before
	public void setUp(){
		
		titulo.getContaBancaria().setBanco(BancosSuportados.BANCO_MONEY_PLUS.create());
		titulo.getContaBancaria().setAgencia(new Agencia(57, "1"));
		titulo.getContaBancaria().setCarteira(new Carteira(10));
		...
		setCampoLivreEsperadoComoString("0057100001234567800123450");
	}

	@Test
	public void testWriteParaCarteirasQueNaoPrecisamDeContaEAgencia() {
		
		titulo.getContaBancaria().setCarteira(new Carteira(12));
		
		createCampoLivreToTest();
		
		assertEquals(25, writeCampoLivre().length());
		assertEquals("0057120001234567800123450", writeCampoLivre());
	}
}
